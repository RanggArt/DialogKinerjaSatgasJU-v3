...
function bukaModalDenganKonfirmasi(id) {
  ...
  // Jika foto belum diupload, munculkan Message Box Konfirmasi
  if (!st.foto) {
    const konfirmasi = confirm("Anda belum melampirkan foto kegiatan. Apakah Anda ingin tetap mencetak bukti tanpa foto?");
    
    // Jika user klik "Cancel" / "Batal", batalkan proses modal
    if (!konfirmasi) {
      return; 
    }
  }

  // Jika ada foto ATAU user klik "OK" pada konfirmasi, langsung buka modal cetak seperti biasa
  bukaModal(id);
}
...
function cetakDokumen() {
  ...
  // Gunakan instansiasi baru html2pdf setiap kali cetak agar teks tidak melorot
  const pekerjaCetak = html2pdf();
  ...
  const opsi = {
    ...
    html2canvas:  { 
      ...
      useCORS: true, 
      logging: false,
      removeContainer: true // Menghapus container sampah otomatis agar teks tidak turun
    },
    jsPDF:        { unit: 'mm', format: 'legal', orientation: 'portrait' }
  };

  // 1. Jalankan download PDF menggunakan pekerja baru
  pekerjaCetak.set(opsi).from(elemen).save();

  // 2. Gunakan jeda singkat untuk menutup modal secara aman tanpa merusak DOM HTML
  setTimeout(function() {
    tutupModal();
  }, 300); 
}
